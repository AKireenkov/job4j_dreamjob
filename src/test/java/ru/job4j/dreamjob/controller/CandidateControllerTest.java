...
class CandidateControllerTest {
    ...
    public void whenPostCandidateWithFileThenSameDataAndRedirectToCandidatesPage() throws Exception {
        ...
        when(candidateService.save(candidatesArgumentCaptor.capture(), fileDtoArgumentCaptor.capture())).thenReturn(candidate);
    ...
    @Test
    public void whenRequestGetCandidateByIdThenGetCandidatePage() {
        var candidate1 = new Candidate(1, "test1", "desc1", now(), 1, 2);
    ...
    public void whenUpdateCandidateWithFileThenSameDataAndRedirectToCandidatesPage() throws Exception {
        var replacedCandidate = new Candidate(1, "test2", "desc2", now(), 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        ...
        var model = new ConcurrentModel();
        var view = candidateController.update(replacedCandidate, testFile, model);
        var actualCandidate = candidateArgumentCaptor.getValue();
        var actualFileDto = fileDtoArgumentCaptor.getValue();

        assertThat(view).isEqualTo("redirect:/candidates");
        assertThat(actualCandidate).isEqualTo(replacedCandidate);
        assertThat(fileDto).usingRecursiveComparison().isEqualTo(actualFileDto);

    }

    @Test
    public void whenDeleteCandidateThenDoesNotExist() {
