...
public class VacancyControllerTest {
    ...
    @Test
    public void whenRequestGetVacancyByIdThenGetVacancyPage() {
        var vacancy1 = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
    ...
    public void whenUpdateVacancyWithFileThenSameDataAndRedirectToVacanciesPage() throws Exception {
        var replacedVacancy = new Vacancy(1, "test2", "desc2", now(), true, 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var vacancyArgumentCaptor = ArgumentCaptor.forClass(Vacancy.class);
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
        ...
        var model = new ConcurrentModel();
        var view = vacancyController.update(replacedVacancy, testFile, model);
        var actualVacancy = vacancyArgumentCaptor.getValue();
        var actualFileDto = fileDtoArgumentCaptor.getValue();

        assertThat(view).isEqualTo("redirect:/vacancies");
        assertThat(actualVacancy).isEqualTo(replacedVacancy);
        assertThat(fileDto).usingRecursiveComparison().isEqualTo(actualFileDto);

    }

    @Test
    public void whenDeleteVacancyThenDoesNotExist() {
